  ...
  const handleRequestReset = async (event: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/auth/request-password-reset", {
        ...
        credentials: "include",
        body: JSON.stringify({ identifier, accountType: "tenant" }),
      });
  ...
  const handleResetPassword = async (event: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/auth/reset-password", {
        ...
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ token, newPassword, accountType: "tenant" }),
      });
      const data = await response.json();
      if (!response.ok) {
    ...
    <div className="min-h-[calc(100vh/var(--ui-zoom))] h-[calc(100vh/var(--ui-zoom))] flex flex-col items-center justify-center bg-background relative overflow-hidden">
      ...
      <Card className="w-full max-w-md shadow-2xl">
        <CardHeader className="text-center space-y-2">
          <CardTitle>Tenant Password Reset</CardTitle>
          <CardDescription>
            {token ? "Set a new password for your tenant account." : "Request a password reset link."}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {token ? (
